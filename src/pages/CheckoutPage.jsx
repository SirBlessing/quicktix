...
export default function CheckoutPage() {
  ...
  const [event,   setEvent]   = useState(null)   // ← null until API responds
  ...
  useEffect(() => {
    client.get(`/events/${id}`)
      ...
      .catch(() => setError('Could not load this event. Please go back and try again.'))
  ...
  if (loading) return (
    <div className="page">
      <div style={{ textAlign:'center', padding:'80px 0' }}>
        ...
        <p style={{ color:'var(--txt3)', marginTop:14, fontSize:14 }}>Loading event…</p>
      </div>
    </div>
  )
  ...
  if (!event && !loading) return (
    <div className="page">
      <div style={{ textAlign:'center', padding:'80px 24px' }}>
        ...
        <p style={{ color:'var(--txt3)', marginBottom:24 }}>{error || 'This event may have been removed.'}</p>
    ...
    <div className="page">
      <div style={{ textAlign:'center', padding:'80px 24px' }}>
        ...
        <h2 style={{ fontSize:20, fontWeight:700, marginBottom:8 }}>
          {event.isFree ? 'Registering you...' : 'Redirecting to Paystack...'}
        </h2>
              ...
              <form onSubmit={pay} style={{ display:'flex', flexDirection:'column', gap:15 }} noValidate>
                {[
                  ...
                  <div key={k} className="form-group">
                    ...
                    <input
                      className="input" type={t} placeholder={ph}
                      value={form[k]}
                      onChange={e => setForm({ ...form, [k]: e.target.value })}
                    />
                    {k === 'attendeeEmail' && (
                      <p className="hint">📧 Your ticket will be sent to this email</p>
                    )}
                  </div>
                ))}
              </form>
            ...
            {!event.isFree && (
              <div className="card checkout-card">
                ...
                <div className="pay-method selected">
                  ...
                  <div>
                    <p className="pay-name">Pay with Paystack</p>
                    <p className="pay-desc">Cards, Bank Transfer, USSD, QR Code</p>
                  </div>
                  <div className="pay-radio"><div className="pay-radio-dot" /></div>
                </div>
                <p className="secure-note">
                  🔒 Your payment is secured by Paystack. QuickTix never stores your card details.
                </p>
              </div>
            )}
          </div>
            ...
            <div className="card order-card">
              ...
              <div className="order-lines">
                <div className="order-line">
                  <span>{event.isFree ? 'Registration' : `Ticket × ${qty}`}</span>
                  <span>{event.isFree ? 'FREE' : `₦${sub.toLocaleString()}`}</span>
                </div>
                {!event.isFree && (
